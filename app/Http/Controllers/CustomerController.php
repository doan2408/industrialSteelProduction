...
class CustomerController extends Controller {
    ...
    public function data(Request $request){
		...
		$sql_data = tblCustomerModel::select('tbl_customer.*');
			...
			if (count($r_query) > 0) {
				if(isset($r_query['value'])){
					...
					$sql_data->where(function($query) use ($q_search) {
						$query->where('tbl_customer.name', 'LIKE', '%' . $q_search . '%');
					}); 
				}
			}
		}		
		...
		if ($request->filter_status != 'all' && $request->filter_status != '') {			
			$sql_data->where('tbl_customer.status',$request->filter_status);
		}else{
			$sql_data->where('tbl_customer.status',1);
		}
		...
		if(count($data)>0){
			...
			foreach($data as $item){
				...
				$list[] = ['id'=>$item->id,'name'=>$item->name,'number'=>$number,'status'=>$status,'tstatus'=>$item->status
							...
							'url_view'=>'',
							'url_edit'=>route('edit-customer',$item->id),
							'url_active'=>route('post-active-customer',$item->id),
							'url_delete'=>route('post-delete-customer',$item->id)];
				$number++;
			}
	...
	public function update(Request $request){
		...
		$validator = Validator::make($input_all, $rules, $messages, $field);
		if (!$validator->fails()) {
			$data = tblCustomerModel::find($request->id);
	...
	public function active(Request $request,$id){
		tblCustomerModel::where('id',$id)->update(['status'=>1]);
		return 'true';
	}
