...
class ProjectController extends Controller {
    ...
    public function data(Request $request){
		...
		$sql_data = tblProjectModel::select('tbl_project.*');
			...
			if (count($r_query) > 0) {
				if(isset($r_query['value'])){
					...
					$sql_data->where(function($query) use ($q_search) {
						$query->where('tbl_project.name', 'LIKE', '%' . $q_search . '%');
					}); 
				}
			}
		}		
		...
		if ($request->filter_status != 'all' && $request->filter_status != '') {			
			$sql_data->where('tbl_project.status',$request->filter_status);
		}
		...
		if(count($data)>0){
			...
			foreach($data as $item){
				...
				$list[] = ['id'=>$item->id,'name'=>$item->name,'number'=>$number,'status'=>$status,'tstatus'=>$item->status
							...
							'url_view'=>'',
							'url_edit'=>route('edit-project',$item->id),
							'url_active'=>route('post-active-project',$item->id),
							'url_delete'=>route('post-delete-project',$item->id)];
				$number++;
			}
	...
	public function create(){	
		$product = tblProductModel::where('status',1)->get();
		return view('admin.project.add')->with('product',$product);
	}
	public function store(Request $request){
		...
		if (!$validator->fails()) {			
			...
			$check = tblProjectModel::where('slug',CommonModel::gen_slug($request->name))->first();
			if($check){
			...
			if($request->has('product_id')){
				if(count($request->product_id)>0){
					$data->list_p = implode(',',$request->product_id);
				}
			}
	...
	public function edit($id){
		$data = tblProjectModel::find($id);
		if($data){		
			$product = tblProductModel::where('status',1)->get();
			$data_seo = tblSeoModel::where('id',$data->seo_id)->first();
			$list_p=[];
			if($data->list_p){
				$list_p = explode(',',$data->list_p);
			}
			return view('admin.project.edit')->with('data_seo',$data_seo)->with('data',$data)->with('product',$product)->with('list_p',$list_p);
		}else{
			echo 404;
		}
	}
	public function update(Request $request){
		...
		if (!$validator->fails()) {
			...
			$data = tblProjectModel::find($request->id);
	...
	public function active(Request $request,$id){
		tblProjectModel::where('id',$id)->update(['status'=>1]);
		
		return 'true';
	}
	public function remove(Request $request,$id){
		tblProjectModel::where('id',$id)->update(['status'=>2]);
