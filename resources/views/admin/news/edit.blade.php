							...
							<div class="row">
								...
								<div class="col-12 col-lg-8">
									<div class="mb-3">
										...
										<select class="js-example-placeholder-multiple" name="cate_id">
											...
											@if($item->parent==0)
											<option value="{{$item->id}}" @if($item->id == $data->cate_id) selected @endif>{{$item->name}}</option>
												@foreach($category as $item1)
												@if($item1->parent==$item->id)
												<option value="{{$item1->id}}" @if($item1->id == $data->cate_id) selected @endif>-{{$item1->name}}</option>
												@endif
												@endforeach
											@endif
											@endforeach												
								...
								<div class="col-12 col-lg-12">
									<div class="mb-3">
										...
										{{Form::text('tags',$data->tags,array('class'=>'form-control tags_input'))}}
									</div>
								</div>
							</div>
