...
<div class="container-fluid">
	<form class="form-wizard" method="post" action="{{route('post-edit-product')}}" enctype="multipart/form-data">
							@csrf
	<!-- start page title -->
						...
						@include('admin.alert')
							...
							<div class="row">	
								<div class="col-12 col-lg-12">
									<div class="mb-3">
										<label class="form-label">Ảnh 319x179<img id="avatar_img" src="{{$data->avatar}}" width="100px"/></label>	
										<input style="display:none" class="form-control file_upload_server_custom" id="label_avatar" type="file" data-img="avatar_img" data-id="avatar" data-url="{{route('dashboard-upload-one')}}" accept="image/*">
								...
								<div class="col-12 col-lg-12">
									<div class="mb-3">
										...
										<div class="input-group"><label style="margin-bottom:0px;" class="input-group-text" for="label_pdf">Upload file</label>
											{{Form::text('pdf',$data->pdf,array('class'=>'form-control input-group-air ','id'=>'pdf'))}}
										</div>
									</div>
								</div>
								<div class="col-12 col-lg-12">
									<div class="mb-3">
										...
										<select class="js-example-placeholder-multiple" name="cate_id[]" multiple>
											...
											@if($item->parent==0)
											<option value="{{$item->id}}" @if(in_array($item->id,$selected)) selected @endif>{{$item->name}}</option>
												@foreach($category as $item1)
												@if($item1->parent==$item->id)
												<option value="{{$item1->id}}" @if(in_array($item1->id,$selected)) selected @endif>-{{$item1->name}}</option>	
												@endif
												@endforeach		
											@endif
											@endforeach												
								...
								<div class="col-12 col-lg-12">
									<div class="mb-3">
										...
										{{Form::text('content',$data->content,array('class'=>'form-control tinymce_editor','data-upload-url'=>route('dashboard-upload-multi')))}}
									</div>
								</div>
								<div class="col-12 col-lg-12">
									<div class="mb-3">
										<label class="col-form-label pt-0">Ứng dụng sản phẩm</label>
										{{Form::text('used',$data->used,array('class'=>'form-control tinymce_editor','data-upload-url'=>route('dashboard-upload-multi')))}}
									</div>
								</div>
								<div class="col-12 col-lg-12">
									<div class="mb-3">
										<label class="col-form-label pt-0">Thông số</label>
										{{Form::text('specification',$data->specification,array('class'=>'form-control tinymce_editor','data-upload-url'=>route('dashboard-upload-multi')))}}
									</div>
								</div>
							</div>	
						</div>
							...
							<div class="card-body">
								...
								<div class="image_upload_multi_content row">
									...
									<div class="col-12 col-lg-3 image_insert_caption">										
										<img class="content-img" style="width: 100%;" src="{{$item}}">
										<input type="hidden" value="{{$item}}" name="image_upload_multi_file[]">							
